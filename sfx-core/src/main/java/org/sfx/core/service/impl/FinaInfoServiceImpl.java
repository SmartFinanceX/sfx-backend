...
public class FinaInfoServiceImpl implements FinaInfoService {
    ...
    public ResponseResult getFinanceReportInfo(String ticker, Short category) {
        QueryWrapper<FncReportInfo> fncReportInfoQueryWrapper = new QueryWrapper<>();
        fncReportInfoQueryWrapper.eq("ticker", ticker).eq("category", category);
        try {
            List<FncReportInfo> fncReportInfos = fncMapper.selectList(fncReportInfoQueryWrapper);
